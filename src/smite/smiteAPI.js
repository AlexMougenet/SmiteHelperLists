...
class Smite {
  ...
  getGods (langId) {
    let url = util.genUrl(this.smiteUrl, 'getgods', this.devId, this.authKey, process.env[`SMITE_${this.platform}_SESSION`]) + '/' + langId
    return new Promise(function (resolve, reject) {
      request(url, (error, response, body) => {
        if (!error && response.statusCode === 200) {
          resolve(JSON.parse(body))
  ...
  getGodSkins (godId, langId) {
    let url = util.genUrl(this.smiteUrl, 'getgodskins', this.devId, this.authKey, process.env[`SMITE_${this.platform}_SESSION`]) + '/' + godId + '/' + langId
    return new Promise(function (resolve, reject) {
      request(url, (error, response, body) => {
        if (!error && response.statusCode === 200) {
          resolve(JSON.parse(body))
  ...
  getGodRecommendedItems (godId, langId) {
    let url = util.genUrl(this.smiteUrl, 'getgodrecommendeditems', this.devId, this.authKey, process.env[`SMITE_${this.platform}_SESSION`]) + '/' + godId + '/' + langId
    return new Promise(function (resolve, reject) {
      request(url, (error, response, body) => {
        if (!error && response.statusCode === 200) {
          resolve(JSON.parse(body))
  ...
  getItems (langId) {
    let url = util.genUrl(this.smiteUrl, 'getitems', this.devId, this.authKey, process.env[`SMITE_${this.platform}_SESSION`]) + '/' + langId
    return new Promise(function (resolve, reject) {
      request(url, (error, response, body) => {
        if (!error && response.statusCode === 200) {
          resolve(JSON.parse(body))
